...
| Familiar persona | `extensions/familiar.ts`, `extensions/void-blackhole.ts`, `.familiar-enabled` | Optional `/familiar` Harimo persona: the void still owns startup splash + prompt-page harimo wordmark; when the flag exists, Harimo owns only the below-editor status widget. |
...
| LLM usage atlas | `scripts/llm-usage-dashboard.py`, `extensions/llm-dashboard.ts`, `scripts/check-llm-dashboard.mjs`, `.gitignore`, `schema/manifest.json` | Generates a self-contained browser dashboard (Pi Atlas-inspired tabs/ranges/tables) aggregating Pi session JSONL, Hermes SQLite state, and Claude Code JSONL on both local and the EC2 peer (`ec2-user@10.30.128.24`). `/llm-dashboard` and `/llm-usage` wrap the generator from Pi; generated `dashboard/` output is gitignored because it summarizes private local/remote usage metadata. |
...
| Void blackhole | `extensions/void-blackhole.ts` | Animated ASCII black-hole TUI landing page / `/void` screensaver with a Newtonian accretion-disk simulation; on startup it keeps the post-splash harimo wordmark header and mounts the Harimo status widget when `.familiar-enabled` exists. |
...
harimo wordmark header after keypress, and mounts the Harimo status widget when
