...
| Knowledge compounding | `extensions/knowledge-compound.ts` | Buffers substantive `graph` query/explain results and flushes them on shutdown into `graphify save-result --outcome useful` plus draft oracle notes under `oracle/_raw/`; resolves the nearest project graph, falling back to the pi agent config graph so graph answers compound even from `$HOME` or unrelated cwd. |
