...
**Peer/doctor model repin: `peer` → `openai/gpt-5.5:high`, `doctor` → `anthropic/claude-sonnet-5:high`.**
User-directed. `peer` (gate-tier verification) moves from `google/gemini-3.5-flash:high` to
`openai/gpt-5.5:high` (locked high reasoning); `doctor` (acceptance-path runner) moves from
